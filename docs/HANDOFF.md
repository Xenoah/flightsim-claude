...
### 遊びのループ（一時停止・やり直し・墜落・音）— 完了

2026-09-01 完了。**踏んだ地雷と、判断の理由を残す。**

- **やり直しは記録も一緒に戻すこと。** 機体だけ戻すと、失敗を繰り返すほど
  飛行距離と着陸回数が増えて、記録が「何回やり直したか」を測る数字になる。
  リプレイの記録も捨てる（やり直しは記録に残らないので、残すと再生が別の飛行になる）
- **墜落の閾値は認証資料の「壊れる値」ではない。** 根拠と限界は
  `crates/flightsim-sim/src/crash.rs` の doc に全部書いてある。
  触るときはそこを読むこと。**難易度で変えないこと**（採点ではなく結果）
- **音の波形はコードで合成している**（[ADR-0009](adr/0009-synthesised-audio.md)）。
  音声ファイルを持ち込むと、追加機体と同じ「調達待ち」になる。
  **鳴らない不具合は静かに起きる**（WAV が壊れても rodio は無音になるだけ）ので、
  ログの `audio: ... is playing` で再生に届いたかを確かめること
- **ループの継ぎ目に注意。** ノイズのクロスフェードは「先頭に末尾の続きを混ぜる」。
  逆向きに書くと繋がらない（実測で継ぎ目が 2.48 倍残った）
- **画面を見ないと分からない不具合が、また 2 件出た。** 一時停止中と墜落中に
  「押しても効かないキー」を指示していた。リプレイで直したのと同じ筋を
  2 回踏んでいる。**新しい状態を足したら、その状態で画面を撮ること**
- **診断ログを足した後に clippy を回し忘れて CI を落とした。** 最後の編集の
  後に必ず全部回すこと

### 音（`flightsim-audio`）— 作り直し済み

2026-09-02 に全面的に作り直した。**最初の版はビープだった。** その理由と、
今の作りで守るべきことを残す。

- **正弦波の重ね合わせでエンジンの音は作れない。** エンジンの音は排気の
  圧力パルスの連なりで、正弦波を足すと周波数が合っていてもオルガンになる
- **波形 1 本を再生速度で回してはいけない。** 音色まで一緒に移調して
  「テープの早送り」になる。実機は回転を上げても共鳴は動かない
- 今は Pulse-Train-Resonator（[ADR-0009](adr/0009-synthesised-audio.md)）。
  **回転数はパルスの間隔だけを変え、共鳴（フォルマント）はその場に留まる。**
  ここを回転数に追従させると電子音に戻る
- **音の thread では鍵を使わない。** 待たされた瞬間に出力が途切れる。
  受け渡しは `source::SharedSound` の原子変数
- **検査は数字で。** `dsp::tests_support::magnitude_at`（Goertzel 法）で
  狙った周波数のエネルギーを測る。「それらしい音がする」では検査にならない
- 聞くには `cargo run -p flightsim-audio --example render_engine -- flight.wav`

**踏んだ間違い:**

1. 気筒をエンジンサイクルではなく**点火周波数**の中に並べ、パルスが気筒数ぶん
   速くなっていた（2400 rpm で 80 Hz のはずが 320 Hz）
2. 共振器の正規化を**白色雑音**で測ろうとした。中心周波数が高いほど帯域が広く
   通る雑音の電力が増えるのは正しい挙動で、正規化ができていても揃わない。
   中心周波数の正弦波で測ること
3. クリックの検査を**絶対値**で置いた。音自体が 1 標本あたり 0.05 程度は動く
