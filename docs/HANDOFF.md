...
更新: 2026-08-06（TASK-3 統合ランナー完了 = **M1 完了**）
...
| `flightsim-sim` | 地形と FDM の結線。ヘッドレスで実地形の上を飛べる |

**M1 は完了した。** 焼いたタイルの上で離陸 → 旋回 → 着陸が通り、軌跡を CSV で出せる。
**次は M2（描画）。**
