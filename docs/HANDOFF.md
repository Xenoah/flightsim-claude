...
**タービン（戦闘機）の音を足したときに踏んだ間違い:**

1. **バズソーのパルスが 1 回転あたり翼数ぶん繰り返されていた。** `rem_euclid` で
   周期関数にしたため、軸回転ではなく翼通過の周波数になった。**バズソーの定義
   そのものが崩れる**（軸回転の倍音に並ぶからバズソーなので）
2. **そのパルスが直流成分を持っていた。** 32 枚ぶん足して出力が 4.5 まで振り切れた
3. **強度（速度の 6 乗）をそのまま振幅に使った。** 振幅は 3 乗。アフターバーナーで
   10 倍以上に跳ねる
4. **検査が実際の翼通過音（7077 Hz）を外して 7000 Hz を見ていた。** 1 Hz 分解能では
   完全に外れ、「タービンの方が暗い」という誤った結果を出した。**鋭い音は
   狙い撃ちの周波数ではなく帯域で測ること**
5. **「キーン」と「笛」は紙一重。** ファンを厚くしすぎると 4 kHz 以上が 98% を
   占めて単なる高い笛になる。**排気の広帯域と釣り合って初めてジェットに聞こえる**
...
### 昇降舵トリム — 追加済み（2026-09-06）

「S を離すと墜落する」という報告への対処。**踏まえておくべきこと:**

- **舵中立の釣り合い速度は約 107 kt。** `Cm = pitch_zero + pitch_alpha·α +
  pitch_elevator·δe = 0` から迎角 0.97 度。離陸直後の 75 kt から手を離すと、
  そこへ向けて機首を下げる。**発散ではなく、減衰する長周期振動**で、
  高度があれば落ち着く。**足りなかったのはトリムだけ**だった
- 既定 0.09（約 75 kt）。**これを変えるときは
  `crates/flightsim-sim/tests/hands_off.rs` の `DEFAULT_TRIM` も直すこと**
  （2 箇所にある。片方だけ直すと検査が意味を失う）
- **既定を強くしすぎると離陸を邪魔する。** 0.15 では滑走中に機首が 17 度まで
  上がり、60 m へ到達しなくなった。検査で上限を押さえてある
- **自動トリムは採らなかった。** 迎角を目標に追わせる積分制御を試したが、
  全開のまま迎角を保つと長周期振動が増幅し、227 m まで上がったあと失速して
  背面から落ちた。実測して捨てている
- **「落ちなくなる」とは書かないこと。** 45 kt・機首上げ 40 度からの回復には
  145 m 要る。60 m しか無ければ間に合わない。これは物理

### コックピット内装 — 追加済み（2026-09-06）

Cessna 172 の実寸から手続き的に組んである（`flightsim-render::cockpit`）。
**踏まえておくべきこと:**

- **寸法はインチの定義から計算する。** 丸めたメートル値を書くと出典と
  端数がずれる（一度そうなった）。`INCH` 定数を通すこと
- **内装は目の位置（`CameraRig::eye_offset`）を基準に置く。** 重心基準に
  すると、目を動かしたときに顔が計器盤にめり込む
- **2D の計器と 3D の座ぐりは対で決めてある。**
  `cockpit::SIX_PACK_DROP`（目から 0.25 m 下）と
  `instruments` の `bottom: 93 px`。片方だけ動かすと計器が盤から浮く
- **視野角 60 度は人の視野より狭い。** 実機どおりの位置に置くと計器盤も
  操縦輪も画面外へ出る。SIX_PACK_DROP は実機の再現ではなく画面に収める
  ための妥協で、そう明記してある。操縦輪は幾何どおりに置いてあるので
  **この視野角では見えない**

**踏んだ間違い:**

1. **カメラの目線オフセットが機体軸を組み替えていた。** `eye_offset` を
   `(前, -下, -右)` に並べ替えて `aircraft.rotation`（機体軸 → 描画座標）に
   掛けており、既定の `[0.6, 0, -0.9]` では目が「上 0.9 m」ではなく
   **「右 0.9 m」** に置かれていた。**内装を入れるまで誰も気付けなかった**
   （箱を並べただけの外形では上下と左右の取り違えが見えない）
2. **近クリップ面が 0.5 m だった。** 目から 0.48 m にある操縦輪が
   切り取られて消えていた。内装を入れるまで 0.5 m より近いものが無かった
