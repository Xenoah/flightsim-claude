...
- **音** — エンジン音・風切り音・**失速警報**。波形は録音でも同梱ファイルでもなく、
  **毎標本その場で合成する**（[ADR-0009](docs/adr/0009-synthesised-audio.md)）。
  既定は**戦闘機の低バイパス比ターボファン**: ファンの翼通過音（32 枚 × N1/60 で
  ...
  **音は機体の飛び方には影響しない** — FDM が解いているのは 160 hp のピストン単発
...
- **コックピット内装** — Cessna 172 の実寸から手続き的に組んである
  ...
  配布物に第三者の素材が入らない。**左席に座る**（172 は左が機長席）
- **昇降舵トリム** — `[` / `]`。**手を離したときに釣り合う速度**を決める。
  既定は 0.09（約 75 kt）で、上昇姿勢から手を離してもそのまま上昇を続ける。
  トリムが無いと舵中立の釣り合いが約 107 kt なので、機首を下げて加速しようとし、
...
- **リプレイ** — 飛行は常に記録されていて、`F9` で `.fsreplay` に保存、
  `--replay <FILE>` で再生する。**記録するのは操縦入力とフレーム時間だけ**で、
  再生時は同じ物理を回し直す。姿勢を流し込む方式と違い、再生中も接地判定・
  計器・着陸評価が本物と同じ経路を通る。一時停止・速度変更・10 秒戻しつき
