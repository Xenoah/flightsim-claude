...
- **音** — エンジン音・風切り音・**失速警報**。波形は録音でも同梱ファイルでもなく、
  **毎標本その場で合成する**（[ADR-0009](docs/adr/0009-synthesised-audio.md)）。
  エンジンは点火のたびに排気パルスを立て、排気管と機体の共鳴に通す
  （Pulse-Train-Resonator）。点火周波数は諸元から出す — 4 気筒 4 サイクルなら
  `rpm/30` で、2400 rpm で 80 Hz。**回転数はパルスの間隔だけを変え、共鳴は動かない**
  ので、出力を上げると高さだけでなく音色が硬くなる。
